...
from consumers.models.weather import Weather
from consumers.models.lines import Lines
import consumers.topic_check as topic_check
...
def run_server():
    ...
    if topic_check.topic_exists("SUMMARY") is False:
        logger.fatal(
            "Ensure that the KSQL Command has run successfully before running the web server!"
        )
        exit(1)

    if topic_check.topic_exists("com.opt.stations.lean") is False:
        logger.fatal(
